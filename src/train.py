"""
train.py — FP16 + LoRA fine-tuning for Gemma.
...
def main():
    ...
    parser.add_argument("--packing", action="store_true", default=False, help="Enable sequence packing (requires flash-attn)")
    ...
    # Dynamic hardware precision: BF16 on Ampere/Ada/Hopper (A4500/A5000/4090/A100), FP16 on Turing (T4)
    use_bf16 = torch.cuda.is_bf16_supported() if torch.cuda.is_available() else False
    compute_dtype = torch.bfloat16 if use_bf16 else torch.float16
    precision_name = "BF16" if use_bf16 else "FP16"

    # Multi-threaded dataloader & fused optimizer for GPU acceleration
    num_workers = min(4, max(1, (os.cpu_count() or 1) - 1))
    optim_type = "adamw_torch_fused" if torch.cuda.is_available() else "adamw_torch"

    tokenizer = AutoTokenizer.from_pretrained(args.model_id)

    print(f"[Rank {local_rank}] Hardware detected: Precision={precision_name}, Workers={num_workers}, Optim={optim_type}, Attention={'sdpa' if use_bf16 else 'eager'}")
